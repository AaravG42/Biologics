...
class KGModel(nn.Module, ABC):
    ...
    def get_ranking(self, queries, filters, batch_size=1000):
        ...
        with torch.no_grad():
            ...
            candidate_device = candidates[0].device
            while b_begin < len(queries):
                these_queries = queries[b_begin:b_begin + batch_size].to(candidate_device)
                ...
                for i, query in enumerate(these_queries):
                    ...
                    filter_out = list(filters.get(filter_key, ()))
                    target = int(queries[b_begin + i, 2].item())
                    if target not in filter_out:
                        filter_out.append(target)
                    if filter_out:
                        min_index = min(filter_out)
                        max_index = max(filter_out)
                        if min_index < 0 or max_index >= scores.shape[1]:
                            raise IndexError(
                                "Filter index out of bounds for ranking: "
                                f"query={tuple(int(x) for x in query.tolist())} "
                                f"filter_key={filter_key} "
                                f"target={target} "
                                f"min_filter={min_index} "
                                f"max_filter={max_index} "
                                f"num_entities={scores.shape[1]}"
                            )
                    filter_tensor = torch.tensor(filter_out, dtype=torch.long, device=scores.device)
