#!/usr/bin/env python3
"""Hyperparameter search for directional IMGT clinical-indication ranking."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "by validation clinical-indication tail mean rank on a directional dataset."
        )
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="IMGT_no_leakage_directional_clinical",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Skip trials that already have metrics.json and reuse them in aggregation.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def train_single_trial(
    ...
    relation_offset: int,
    forward_clinical_head_entity_ids: List[int],
    ...
    kg_optimizer = KGOptimizer(
        ...
        verbose=False,
        hard_tail_negatives_by_relation={
            int(forward_relation_id + relation_offset): forward_clinical_head_entity_ids,
        },
    )

    def compute_directional_metrics(split_examples: torch.Tensor) -> Dict[str, Dict[str, float]]:
        forward_examples = filter_triples_by_relation_ids(
            split_examples, [forward_relation_id]
        )
        tail_metrics = compute_rhs_metrics(
            model, forward_examples, rhs_filters, batch_size=eval_batch_size
        )
        head_metrics = compute_lhs_metrics(
            model,
            forward_examples,
            lhs_filters,
        ...
        total_count = int(tail_metrics["count"]) + int(head_metrics["count"])
        if total_count == 0:
            combined = {
                "count": 0,
                "mr": float("inf"),
            ...
            combined = {
                "count": total_count,
                "mr": (
                    (tail_metrics["tail_mr"] * int(tail_metrics["count"]))
                    + (head_metrics["head_mr"] * int(head_metrics["count"]))
                )
                / total_count,
                "mrr": (
                    (tail_metrics["tail_mrr"] * int(tail_metrics["count"]))
                    + (head_metrics["head_mrr"] * int(head_metrics["count"]))
                )
                / total_count,
                "hits@1": (
                    (tail_metrics["tail_hits@1"] * int(tail_metrics["count"]))
                    + (head_metrics["head_hits@1"] * int(head_metrics["count"]))
                )
                / total_count,
                "hits@3": (
                    (tail_metrics["tail_hits@3"] * int(tail_metrics["count"]))
                    + (head_metrics["head_hits@3"] * int(head_metrics["count"]))
                )
                / total_count,
                "hits@10": (
                    (tail_metrics["tail_hits@10"] * int(tail_metrics["count"]))
                    + (head_metrics["head_hits@10"] * int(head_metrics["count"]))
                )
                / total_count,
            }
        return {
    ...
    for epoch in range(int(trial_config["max_epochs"])):
        ...
        valid_metrics = valid_directional_metrics["tail_prediction"]
        test_metrics = test_directional_metrics["tail_prediction"]
        current = {
            "epoch": epoch,
    ...
    best_metrics["clinical_relation_id"] = forward_relation_id
    best_metrics["clinical_relations"] = {
        "forward_tail_prediction": {
            "name": CLINICAL_INDICATION_RELATION,
            "id": forward_relation_id,
        },
        "forward_head_prediction": {
            "query": "?, imgt:hasClinicalIndication, t",
            "relation_offset": relation_offset,
        },
    }
    return best_metrics
...
def run_trial_job(job: Dict[str, object]) -> Dict[str, object]:
    ...
    forward_clinical_head_entity_ids = load_forward_clinical_head_entity_ids(dataset_dir)
    ...
    metrics = train_single_trial(
        ...
        lhs_filters=lhs_filters,
        rhs_filters=rhs_filters,
        forward_relation_id=forward_relation_id,
        relation_offset=relation_offset,
        forward_clinical_head_entity_ids=forward_clinical_head_entity_ids,
...
def main() -> None:
        ...
        for trial_index, trial_config in enumerate(trial_configs):
            ...
            if args.resume and metrics_path.exists():
                try:
                    ...
                    logging.warning(
                        "Failed to read existing metrics at %s; scheduling trial for recompute.",
                        metrics_path,
                    )
                else:
    ...
    logging.info("Resumed %d existing completed trials", resumed_trials)
    logging.info("Prepared %d new trials", len(jobs))
        ...
        try:
            ...
            for future in iterator:
                try:
                    ...
                    failed_trials += 1
                    logging.exception("A trial failed; continuing with remaining trials.")
                    continue
    ...
    summary_payload = {
        ...
        "rank": args.rank,
        "clinical_relation_id": forward_relation_id,
