...
class KGOptimizer(object):
    ...
    def __init__(
            ...
            double_neg,
            verbose=True,
            hard_head_negatives_by_relation=None,
            hard_tail_negatives_by_relation=None):
        """Inits KGOptimizer."""
        self.model = model
        ...
        self.hard_head_negatives_by_relation = {}
        self.hard_tail_negatives_by_relation = {}
        if hard_head_negatives_by_relation:
            for relation_id, candidate_ids in hard_head_negatives_by_relation.items():
                ...
                if candidates.ndim != 1 or candidates.numel() == 0:
                    raise ValueError(
                        "hard_head_negatives_by_relation entries must be non-empty 1D arrays"
                    )
                self.hard_head_negatives_by_relation[int(relation_id)] = candidates
        if hard_tail_negatives_by_relation:
            for relation_id, candidate_ids in hard_tail_negatives_by_relation.items():
                candidates = torch.as_tensor(
                    candidate_ids,
                    dtype=torch.long,
                    device=self.device,
                ...
                if candidates.ndim != 1 or candidates.numel() == 0:
                    raise ValueError(
                        "hard_tail_negatives_by_relation entries must be non-empty 1D arrays"
                    )
                self.hard_tail_negatives_by_relation[int(relation_id)] = candidates
    ...
    def get_neg_samples(self, input_batch):
        ...
        for relation_id, candidates in self.hard_tail_negatives_by_relation.items():
            relation_mask = negative_batch[:, 1] == relation_id
            if not torch.any(relation_mask):
                continue
            if candidates.device != input_batch.device:
                candidates = candidates.to(input_batch.device)
            candidate_positions = torch.randint(
                candidates.shape[0],
                size=(int(relation_mask.sum().item()),),
                device=input_batch.device,
            )
            negsamples[relation_mask] = candidates[candidate_positions].to(
                dtype=input_batch.dtype
            )
        ...
        if self.double_neg:
            ...
            for relation_id, candidates in self.hard_head_negatives_by_relation.items():
                relation_mask = negative_batch[:, 1] == relation_id
                if not torch.any(relation_mask):
                    continue
                if candidates.device != input_batch.device:
