#!/usr/bin/env python3
"""Build KG vocabularies after excluding triples whose object is a literal."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "matrices from Query.csv after dropping triples whose object is a literal."
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/kg_vocab_literal_excluded"),
...
def load_uri_triples(path: Path) -> Tuple[List[Triple], int, int]:
    triples: List[Triple] = []
    dropped = 0
    total = 0
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            total += 1
            head, relation, tail = row["sub"], row["pred"], row["obj"]
            if is_uri(head) and is_uri(tail):
                triples.append((head, relation, tail))
            else:
                dropped += 1
    return triples, dropped, total
...
def write_metadata(
    ...
    metadata = {
        ...
        "literal_policy": "exclude object literals and any non-URI subject/object rows",
...
def main() -> None:
    args = parse_args()
    triples, dropped, all_input_rows = load_uri_triples(args.input)
    ...
    print(f"Loaded {all_input_rows} triples from {args.input}")
    print(f"Kept URI-to-URI triples: {len(triples)}")
    print(f"Dropped literal/non-URI triples: {dropped}")
